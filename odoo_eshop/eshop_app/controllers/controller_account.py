...
def register():  # noqa: C901
    ...
    incorrect_data = False
    # Check First Name
    first_name, error_message = check_first_name(request.form["first_name"])
    if error_message:
        incorrect_data = True
        flash(error_message, "danger")

    # Check Last Name
    last_name, error_message = check_last_name(request.form["last_name"])
    if error_message:
        incorrect_data = True
        flash(error_message, "danger")
    ...
    if error_message:
        ...
        if len(partner_ids) > 1:
            ...
            incorrect_data = True
            partner = execute_odoo_command("res.partner", "browse", partner_ids)[0]
...
def activate_account(id, email):
    result = execute_odoo_command("res.partner", "eshop_email_confirm", id, email)
    ...
    if result == "enabled":
        flash(
            _(
                ...
                " You can now log in to begin to purchase."
            ),
            "success",
        )
        return redirect_url_for("login_view")
...
def password_lost():
    ...
    result = execute_odoo_command("res.partner", "eshop_password_lost", email)

    if result == "too_many_email":
        flash(
            _("There is a problem with your account." " Please contact your seller."),
            "danger",
        )
