...
def sanitize_qty(quantity, allow_null):
    ...
    if not allow_null and quantity == 0:
        return {"state": "danger", "message": _("You can not set a null Quantity.")}
    return {
        "state": "success",
        "quantity": quantity,
    }
...
def get_current_sale_order():
    res = get_odoo_uncached_object("sale.order", get_current_partner_id())
    if res:
        return res[0]
    else:
        return False
...
def set_quantity(product_id, quantity, allow_null, method):
    ...
    if company.eshop_vat_included:
        ...
        res["minimum_ok"] = res["amount_total"] >= company.eshop_minimum_price
    else:
